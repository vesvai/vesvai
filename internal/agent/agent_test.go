...
func (p *mockProvider) ListModels(ctx context.Context) ([]llm.Model, error) {
	return nil, nil
}
...
func (a *mockAgent) ToolNames() []string  { return a.toolNames }
...
func TestRunner_NewRunner(t *testing.T) {
	...
	tr := NewToolRegistry()
	runner := NewRunner(provider, bus, tr)
...
func TestRunner_Run_NilEventBus(t *testing.T) {
	...
	tr := NewToolRegistry()
	runner := NewRunner(provider, nil, tr)
	agent := newMockAgent("test")
...
func TestRunner_Run_WithMiddleware(t *testing.T) {
	...
	tr := NewToolRegistry()
	runner := NewRunner(provider, bus, tr, mw)
	agent := newMockAgent("test")
