...
func TestSubagentTool_Integration_WithRunner(t *testing.T) {
	...
	tr := NewToolRegistry()
	innerRunner := NewRunner(innerProvider, bus, tr)
	...
	outerTr := NewToolRegistry()
	outerTr.Register(subtool)
	outerRunner := NewRunner(outerProvider, bus, outerTr)
	outerAgent := newMockAgent("outer")
	outerAgent.toolNames = []string{"delegate"}
