...
func NewFileStore(workspacePath string) (*FileStore, error) {
	...
	var basePath string
	if workspacePath != "" {
		projectHash := hashWorkspace(workspacePath)
		basePath = filepath.Join(sessionsBase, projectHash)
	} else {
		basePath = sessionsBase
	}
...
func hashWorkspace(workspace string) string {
	h := sha256.Sum256([]byte(filepath.Clean(workspace)))
...
func (s *FileStore) rebuildIndex() error {
	...
	s.index.mu.Lock()
	for _, m := range s.index.sessions {
		knownIDs[m.ID] = true
	}
	s.index.mu.Unlock()
