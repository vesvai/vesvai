...
func TrinityPromptBuilder() *prompt.Prompt {
	return prompt.New().
		...
		XMLTag("example",
			...
			prompt.Paragraph("assistant: [uses grep or glob to find where similar tests are defined, then read relevant files one at a time (one tool per message, wait for each result), then edit or write to add tests]")).
		...
		List("Use the available search tools to understand the codebase and the user's query. Use one tool per message; after each result, decide the next step and call one tool again.",
