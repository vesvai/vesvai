...
func TestRequest_WithStructuredOutput(t *testing.T) {
	schema := map[string]any{"type": "object", "properties": map[string]any{}}
	r := NewRequest("m", nil).WithStructuredOutput("weather", schema)
	if r.ResponseFormat == nil {
		t.Fatal("ResponseFormat is nil")
	}
	if r.ResponseFormat.Type != ResponseFormatJSONSchema {
		t.Errorf("Type = %q, want %q", r.ResponseFormat.Type, ResponseFormatJSONSchema)
	}
	js := r.ResponseFormat.JSONSchema
	if js == nil {
		t.Fatal("JSONSchema is nil")
	}
	if js.Name != "weather" {
		t.Errorf("Name = %q, want weather", js.Name)
	}
	if js.Strict != nil {
		t.Errorf("Strict = %v, want nil (omitted)", *js.Strict)
	}

	data, err := json.Marshal(r.ResponseFormat)
	if err != nil {
		t.Fatalf("json.Marshal error: %v", err)
	}
	var decoded map[string]any
	...
	if decoded["type"] != ResponseFormatJSONSchema {
		t.Errorf("type = %v, want %q", decoded["type"], ResponseFormatJSONSchema)
	}
	if _, exists := decoded["schema"]; exists {
		t.Error("flat schema key should not be emitted")
	}
	jsDecoded, ok := decoded["json_schema"].(map[string]any)
	if !ok {
		t.Fatalf("json_schema = %v, want object", decoded["json_schema"])
	}
	if jsDecoded["name"] != "weather" {
		t.Errorf("json_schema.name = %v, want weather", jsDecoded["name"])
	}
	if _, exists := jsDecoded["strict"]; exists {
		t.Error("strict should be omitted when false")
	}
}

func TestRequest_WithStrictStructuredOutput(t *testing.T) {
	r := NewRequest("m", nil).WithStrictStructuredOutput("weather", map[string]any{"type": "object"})
	js := r.ResponseFormat.JSONSchema
	if js == nil {
		t.Fatal("JSONSchema is nil")
	}
	...
	jsDecoded := decoded["json_schema"].(map[string]any)
	if jsDecoded["strict"] != true {
		t.Errorf("strict = %v, want true", jsDecoded["strict"])
	}
}

func TestRequest_WithStructuredOutput_OverridesFormat(t *testing.T) {
	r := NewRequest("m", nil).
		WithResponseFormat(&ResponseFormat{Type: ResponseFormatJSONObject}).
...
func TestResponseFormat_JSONObject(t *testing.T) {
	data, err := json.Marshal(&ResponseFormat{Type: ResponseFormatJSONObject})
	if err != nil {
		t.Fatalf("json.Marshal error: %v", err)
	}
	var decoded map[string]any
	...
	if decoded["type"] != "json_object" {
		t.Errorf("type = %v, want json_object", decoded["type"])
	}
	if _, exists := decoded["json_schema"]; exists {
		t.Error("json_schema should be omitted")
	}
}
