...
func (m *Manager) collectTools(ctx context.Context, value interface{}, args ...interface{}) interface{} {
	existingTools, ok := value.([]agent.Tool)
	if !ok {
		return value
	}
	...
	return append(existingTools, NewMemoryTools(mem)...)
}

func (m *Manager) RegisterToolsHook() {
	m.hooks.AddFilter(hook.HookToolsCollect, m.collectTools, 45)
}
